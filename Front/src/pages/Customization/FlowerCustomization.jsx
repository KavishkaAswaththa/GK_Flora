...
export default function FlowerCustomization() {
  ...
  const addFlower = async (e) => {
    ...
    formData.append("file", flowerImage);

    try {
      await axios.post("http://localhost:8080/api/flowers/save", formData, {
        withCredentials: true
      });
  ...
  const addWrappingPaper = async (e) => {
    ...
    const formData = new FormData();
    formData.append("file", wrappingImage);

    try {
      await axios.post("http://localhost:8080/api/wrappingPapers/save", formData, {
        withCredentials: true
      });
      ...
      console.error("Error adding wrapping paper:", error);
    }
  };
